...
const WordSwapPuzzle = () => {
  ...
  const initializeWords = (setNumber) => {
    const positions = ['left', 'right', 'bottom-left', 'bottom-right', 'side']; // Remove 'top' from available positions
    let shuffledPositions;
    do {
      shuffledPositions = [...positions].sort(() => Math.random() - 0.5);
    } while (shuffledPositions.some((pos, i) => pos === wordSets[setNumber][i + 1].correctPosition)); // Skip first word

    return wordSets[setNumber].map((word, i) => ({
      ...word,
      position: i === 0 ? 'top' : shuffledPositions[i - 1] // First word always at top, rest shuffled
    }));
  };

  const [words, setWords] = useState(() => initializeWords(1));
